...
use crate::defs::{ StreamSender, StreamReciever };

use crate::{producer::SimpleProducer, defs::DirContent, remote_consumer::RemoteConsumer};
...
impl RemoteOrchestrator {
    ...
    pub fn new(query: String, root_dir: PathBuf) -> (Self, StreamReciever) {
        let (stream_sender, stream_reciever) = mpsc::channel(32);
            ...
            match pulse_option {
                ...
                Some(_)=>{
                    ...
                    tokio::spawn(async move {
                        ...
                        tokio::spawn(async move {
                            RemoteConsumer::consume(stream_sender_clone, consumed_dir_content_vec, query_clone).await;
                        });
                    });
                },
